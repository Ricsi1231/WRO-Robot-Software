...
6. Saves the captured frame as `calibrate.jpg`.
7. Stops the camera.
...
Place the color target in the center of the camera view, run calibration, and compare the printed HSV stats to the thresholds in `VisionConfig`.
...
`calibrate.jpg` is ignored by Git, along with other `.jpg` and `.png` files. Calibration images are local artifacts and should not be committed unless the ignore rules are intentionally changed.
